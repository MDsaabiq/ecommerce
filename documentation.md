...
### Frontend `.env`
```
VITE_REACT_BASE_URL=http://<EC2_PUBLIC_IP>:30500/api
```
Vite bakes this at build time. With NodePort, the browser reaches the backend directly through `:30500`; the frontend is exposed through `:30080`.
...
|----------|-------|
| `PORT` | 5000 |
| `CORS_ORIGIN` | Frontend origin (`http://<EC2_PUBLIC_IP>:30080`) |
...
```
API calls between the separately exposed frontend/backend ports are not tested in this standalone Docker stage; Kubernetes NodePort provides the final external exposure.
...
nodes:
  ...
  - role: worker
```
- `extraPortMappings` maps EC2 host port `30080` → the Kind control-plane container port `30080`, and EC2 host port `30500` → the Kind control-plane container port `30500`.
- These mappings allow external EC2 traffic to reach the Kubernetes NodePort ports.
- No `ingress-ready` label is needed because the application no longer uses an Ingress controller.
...
**`frontend/nginx.conf`** — `try_files` for React Router. No proxy block — the backend is reached directly through its NodePort.
...
| `frontend-service.yaml` | NodePort `30080` → frontend pods port 80 |
| `backend-deployment.yaml` | 2 backend pods, liveness + readiness on `/api/health` |
| `backend-service.yaml` | NodePort `30500` → backend pods port 5000 |
...
- Application NodePort exposure is handled through the Kind `extraPortMappings` for ports 30080 and 30500
...
### Phase 9 — NodePort External Exposure ✅

This project exposes the frontend and backend directly through Kubernetes `NodePort` Services. No Kubernetes Ingress or NGINX Ingress Controller is required.

**Kind host-port mappings:**
```yaml
extraPortMappings:
  - containerPort: 30080
    hostPort: 30080
...
The external request paths are:

```text
EC2 public IP:30080 → Kind control-plane → frontend NodePort → frontend-service → frontend pod :80
EC2 public IP:30500 → Kind control-plane → backend NodePort → backend-service → backend pod :5000
```
...
```bash
kubectl get svc
# frontend-service ... 80:30080/TCP
# backend-service  ... 5000:30500/TCP
```

**Verify application from EC2:**
```bash
curl -I http://localhost:30080
curl http://localhost:30500/api/health
```

The EC2 Security Group must allow inbound TCP `30080` and `30500`.
...
hey -n 10000 -c 100 http://<EC2-2-IP>:30500/api/products
